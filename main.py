...
# Convierte fecha de YYYY-MM-DD a DD-MM-YYYY para mostrar al usuario
# Si recibe None o un formato inesperado, devuelve "Sin fecha"
def formatear_fecha(fecha):
    if fecha is None: # si no hay fecha cargada
        return "Sin fecha" # devolvemos texto legible
    partes = fecha.split("-") # separamos por guion
    if len(partes) == 3: # si tiene el formato esperado
        return partes[2] + "-" + partes[1] + "-" + partes[0] # invertimos a DD-MM-YYYY
    return fecha # si no, devolvemos como esta
...
def crear_orden(clientes, ordenes, ultimo_id_orden, pagos):
    ...
    fecha_visita_input = input("Fecha de visita (DD-MM-YYYY) o Enter para dejar sin fecha: ").strip() # pide la fecha

    if fecha_visita_input == "": # si no ingreso fecha
        fecha_visita = None # guardamos None
    else:
        partes = fecha_visita_input.split("-") # separamos por guion
        if len(partes) == 3 and len(partes[0]) == 2 and len(partes[1]) == 2 and len(partes[2]) == 4:
            fecha_visita = partes[2] + "-" + partes[1] + "-" + partes[0] # convertimos a YYYY-MM-DD para almacenar
        else:
            print("Formato invalido. Se deja sin fecha.") # avisamos si el formato es incorrecto
            fecha_visita = None # guardamos None como fallback
    ...
    ordenes[id_orden] = { # crea la orden en el diccionario
        ...
        "fecha_visita": fecha_visita, # None si no ingresaron fecha
    ...
    print("Numero de orden:", id_orden) # muestra el ID asignado a la orden
...
def mostrar_ordenes_pendientes(ordenes, clientes):
    ...
    for orden in activas: # recorre las ordenes activas
        ...
        print("  Orden:", orden["id"], "| Cliente:", nombre)
        print("  Estado:", orden["estado"], "| Visita:", formatear_fecha(orden["fecha_visita"]))
    ...
    for id_orden in pagos: # recorremos todos los pagos
        ...
        if len(pago["transacciones"]) == 0: # si no hay transacciones todavia
            ...
            for transaccion in pago["transacciones"]:  # recorremos cada pago recibido
                print("    [", formatear_fecha(transaccion["fecha"]), "] $", transaccion["monto"], "-", transaccion["metodo"])
...
def ver_historial_cliente(clientes, ordenes, pagos, tecnicos):
    ...
    for par in pares: # recorremos los pares ya ordenados
        ...
        print("  Fecha:", formatear_fecha(orden["fecha_creacion"])) # fecha de creacion
